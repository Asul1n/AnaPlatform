...
class Root(object):
    def __init__(
        ...
        Blocksize,               # 分组长度
        Round,                   # 轮数（算法总轮数）
        Branch_number,           # 分支数量（Feistel 结构或 SPN 中输入分为几部分）
        Sbox_bit=None,           # 单个 S 盒输入/输出比特数（如 4 位 S-box）
        Sbox_content=None,       # S 盒映射表
        Matrix=None,             # 线性层（扩散层）的矩阵表示，用于描述置换或矩阵乘法
        NonlinearType=None,      # 指明算法中的非线性操作类型
    ):
