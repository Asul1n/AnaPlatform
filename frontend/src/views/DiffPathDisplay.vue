<template>
  <div class="diff-path">
    ...
    <p class="prob-box">the whole Probability is 0.8763157639378506</p>
    <p class="summary">
      共 {{ paths.length }} 条路径
      <!-- ✅ 若有选中路径，显示 sum_probability -->
      <span v-if="selectedPath !== null" class="highlight-info">
        ｜当前路径累计概率：{{ formatSumProb(paths[selectedPath].sum_probability) }}
      </span>
    </p>
    ...
    <svg :width="svgWidth" :height="svgHeight">
      ...
      <template v-if="paths.length > 0">
        <template v-for="(r, i) in paths[0].rounds" :key="'round'+i">
          <text
            :x="getX(i) + boxW / 2"
            y="30"
            text-anchor="middle"
...
import { fetchDiffPaths } from '@/api/diffPath'
...
interface DiffPath {
  name: string
  totalProb: string
  sum_probability: string
  rounds: DiffRound[]
}

const paths = ref<DiffPath[]>([])
const totalProb = ref<number>(0)
const selectedPath = ref<number | null>(null)

async function loadPaths() {
  try {
    const res = await fetchDiffPaths()
    paths.value = res.paths
    totalProb.value = res.total_probability
  } catch (err) {
    console.error('获取路径数据失败：', err)
  }
}
...
function formatSumProb(sum: string) {
  // 格式化显示，例如保留到 8 位小数
